...
module.exports = {
  ...
  async afterCreate(event) {
    const { result } = event; // `result` chứa dữ liệu của shop đã được tạo, bao gồm `id`

    try {
      // Lấy thông tin cần thiết từ shop đã được tạo
      const { id, name, location, owner } = result;

      if (!owner || !name || !location) {
        throw new Error("Thiếu thông tin cần thiết để tạo QR Code.");
      }
      ...
      const qrContent = JSON.stringify({
        ...
        name,
        location,
        owner, // Chứa username của owner
      });

      // Tạo QR Code và lưu tạm vào file
      const qrImagePath = path.join(__dirname, "temp-qr.png"); // Đường dẫn tạm
      await QRCode.toFile(qrImagePath, qrContent);

      // Upload ảnh QR Code vào Strapi Media Library
      const uploadResponse = await strapi.plugins[
        "upload"
      ].services.upload.upload({
        data: {},
        files: {
          path: qrImagePath,
          name: `shop-${id}-qrcode.png`,
          type: "image/png", // MIME type
          size: fs.statSync(qrImagePath).size,
        },
      });

      // Gán file đã upload vào trường qrCode
      if (uploadResponse && uploadResponse[0]) {
        await strapi.entityService.update("api::shop.shop", id, {
          data: {
            qrCode: uploadResponse[0].id,
          },
        });
      }
      ...
      strapi.log.info(
        `QR Code đã được tạo và upload thành công cho shop ID: ${id}`
      );
    } catch (error) {
      strapi.log.error("Lỗi khi tạo QR Code:", error.message);
      throw new Error("Lỗi khi tạo QR Code. Vui lòng thử lại.");
    }
  },
};
